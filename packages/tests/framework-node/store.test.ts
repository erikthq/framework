...
import { createApp, fileRouter, generateStore } from '@erikt/framework'
...
test('lists every file as a path relative to the directory', async () => {
  ...
  assert.deepEqual(visible(files.map(file => file.path)), [
    'broken-conventions/not-found.ts',
    'conventions/error.ts',
    'conventions/not-found.ts',
    'conventions/routes/index.ts',
    'layouts/main.html',
    'public/hello.txt',
