...
import type { ServeHandle } from '@erikt/framework-node'

// Named only where the handler builds its own Response; a route says its type
// by what it returns.
...
test('serves a response over http', async () => {
  const app = newApp().get('/', defineRoute(() => 'hello over http'))
...
test('passes path params through', async () => {
  const app = newApp().get('/users/:id', defineRoute(c => ({ id: c.params.id })))
...
test('passes the query string through', async () => {
  const app = newApp().get('/search', defineRoute(c => c.url.searchParams.get('q') ?? ''))
...
test('forwards request headers', async () => {
  const app = newApp().get('/', defineRoute(c => c.req.headers.get('x-token') ?? 'none'))
...
test('serves the 404 from the app', async () => {
  const app = newApp().get('/', defineRoute(() => 'root'))
...
test('close stops accepting connections', async () => {
  const server = await serve(newApp().get('/', defineRoute(() => 'up')), { port: 0 })
...
test('reads a streamed request body', async () => {
  const app = newApp().post('/upload', async c => {
    ...
    return c.body(JSON.stringify({ chunks: chunks.length, bytes: total }), {
