...
`FileStore` port against Deno's filesystem. Along with `@erikt/framework-node` and
...
In `packages/tests/framework-deno/`, running the same conformance suite as
`@erikt/framework-node`. The harness is still `node --test`: the Node test spawns a real
