...
import type { DirectoryStore, FileEntry } from "@erikt/framework"
...
function locate(root: URL, path: string): URL {
  if (path === "" || path.startsWith("/") || TRAVERSAL.test(path)) {
    throw new TypeError(
      `Refusing to resolve ${JSON.stringify(path)} outside the store`,
    )
  }
    ...
    async list(listOptions = {}) {
      ...
      for (const entry of entries) {
        ...
        if (
          ...
          !extensions.some((item) => path.endsWith(item))
        )
          continue
